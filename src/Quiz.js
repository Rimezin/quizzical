...
export default function Quiz(props) {
    ...
    React.useEffect(() => {
        fetch(apiUrl)
            .then(res => res.json())
            .then(data => setQuestions(data.results.map(questionObj => ({
                ...questionObj,
                isAnswered: false,
                isCorrect: false,
                questionId: nanoid(),
    ...
    function scoreQuestion(questionObj) {
        ...
        if (questionObj.isAnswered) {
            for(let i=0; i < questionObj.answers.length; i++) {
                let ans = questionObj.answers[i]
                if (ans.isChecked && ans.answerLabel === he.decode(questionObj.correct_answer)) {
                    myScore = true;
                }
            }
        }
    ...
    return (
        <div className="container" style={quizStyles}>
            <h2>Quizzical <span className="alpha">({difficulty})</span></h2>
            {renderQuestions}
            {error !== '' && <span style={{
                color: "red",
                fontWeight: "700",
            ...
            <div className="container-finish">
                ...
                {submitted && <button className="score">Score:<br />{score}/10</button>}
            </div>
        </div>
    )
}
